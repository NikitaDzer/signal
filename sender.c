...
static int signal_receiver( const pid_t pid, const int rel_addr)
{
        union sigval sv = { 0 };
...
static int get_rel_addr( const int n_pages)
{
        return ((double)rand() * (n_pages - 1) / RAND_MAX) * getpagesize()
                + (double)rand() * getpagesize() / RAND_MAX;
}

static int write_shm( char *const shm)
{
        const char msg[] = "This is my message to the whole world.\n"
                           "I want to say, that...\n";
...
static int write_log( FILE *const log, 
                      const int counter, const int rel_addr)
{
