...
static char *read_shm( const char *const addr)
{
...
static int write_log( const int rel_addr, const char *const buffer)
{
        const int nbytes = fprintf( log, "%d: %d: %s"
                                         "------\n",
                                         counter, rel_addr, buffer);
...
static int signal_prepared( void)
{
        union sigval sv = { 0 };
...
static void handle_signal( int sig, siginfo_t *si, void *ucontext)
{
...
int receiver( const char *const log_path, 
